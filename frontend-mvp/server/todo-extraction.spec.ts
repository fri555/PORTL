...
describe('todo extraction contract', () => {
  it('embeds the agreed machine-readable prompt without organization or invented people', () => {
    ...
    expect(TODO_EXTRACTION_SYSTEM_PROMPT).toContain('紧急、较高、普通、较低')
  ...
  it('normalizes all four DingTalk priority levels without merging urgent and high', () => {
    const result = normalizeTodoExtraction([
      { title: '处理线上故障', priority: '紧急' },
      { title: '提交评审材料', priority: '较高' },
      { title: '整理常规周报', priority: '普通' },
      { title: '调研储备方案', priority: '较低' },
    ])

    expect(result.map((item) => item.priority)).toEqual(['urgent', 'high', 'normal', 'low'])
  })
