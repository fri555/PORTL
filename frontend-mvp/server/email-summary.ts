...
export function paginateEmailUids(uids: Array<number | string>, requestedLimit: number, cursor?: string) {
  const limit = Math.max(1, Math.min(50, Math.floor(requestedLimit) || 20))
  const beforeUid = cursor ? Number(cursor) : Number.POSITIVE_INFINITY
  const eligible = uids
    .map(Number)
    .filter((uid) => Number.isFinite(uid) && uid < beforeUid)
    .sort((left, right) => left - right)
  const selected = eligible.slice(-limit).reverse()
  const hasMore = eligible.length > selected.length
  return {
    selected,
    hasMore,
    nextCursor: hasMore && selected.length ? String(Math.min(...selected)) : undefined,
  }
}
...
async function loadEmails(config: EmailServerConfig, options: EmailPageOptions): Promise<EmailPayload> {
  if (!config.host || !config.username || !config.password) {
    return { connected: false, refreshedAt: new Date().toISOString(), items: [], accessIssues: ['企业邮箱连接参数不完整'], hasMore: false }
  }
  ...
  try {
    ...
    try {
      ...
      const page = paginateEmailUids(uids, options.limit, options.cursor)
      const items: EmailSummaryItem[] = []
      for (const uid of page.selected) {
      ...
      return { connected: true, refreshedAt: new Date().toISOString(), items, accessIssues: [], hasMore: page.hasMore, nextCursor: page.nextCursor }
    } finally {
      lock.release()
    }
  } finally {
...
function handler(config: EmailServerConfig): Connect.NextHandleFunction {
  return async (request, response, next) => {
    ...
    try {
      ...
      response.end(JSON.stringify(await loadEmails(config, { limit, cursor })))
    } catch (error) {
