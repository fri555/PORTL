...
function optionalIsoDate(body: JsonRecord, key: string) {
  const value = optionalString(body, key, 40)
  if (value && Number.isNaN(new Date(value).getTime())) throw new Error(`${key} 时间格式无效`)
  return value
}

export function buildTodoReminderArgs(taskId: string, reminderTime: string) {
  return ['todo', 'task', 'add-reminder', '--task-id', taskId, '--base-time', 'customTime', '--reminder-time-stamp', reminderTime]
}

export function todoPriorityValue(priority: string) {
  const value = ({ urgent: '40', high: '30', normal: '20', low: '10' } as Record<string, string>)[priority]
  if (!value) throw new Error('priority 仅支持 urgent、high、normal、low')
  return value
}
...
async function executeAction(body: JsonRecord) {
  ...
  if (action === 'todo') {
    ...
    const reminderTime = optionalIsoDate(body, 'reminderTime')
    if (reminderTime && new Date(reminderTime).getTime() >= new Date(due).getTime()) throw new Error('提醒时间必须早于截止时间')
    ...
    const priorityValue = todoPriorityValue(priority)
    ...
    if (reminderTime && taskId) {
      try {
        await runDws(buildTodoReminderArgs(taskId, new Date(reminderTime).toISOString()))
      } catch {
        warnings.push('提醒时间未能同步')
      }
    } else if (reminderTime) warnings.push('提醒时间未能同步')
    const note = description.slice(0, 2000)
    if (note && taskId) {
      try {
        await runDws(['todo', 'comment', 'add', '--task-id', taskId, '--content', note])
      } catch {
        warnings.push('描述未能同步')
      }
    } else if (note) warnings.push('描述未能同步')
