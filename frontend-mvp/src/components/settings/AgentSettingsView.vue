...
const draft = reactive({
  ...
  version: 1,
  category: '通用助手',
  applicableDepartments: ['通用'] as string[],
...
function toggleAgentStatus(agent: AgentRow) {
  agent.status = agent.status === 'active' ? 'disabled' : 'active'
  agent.updatedAt = '刚刚'
  notify(agent.status === 'active' ? `${agent.name}已启用` : `${agent.name}已停用`)
}
...
function toggleLimited(list: string[], value: string, limit: number) {
  const index = list.indexOf(value)
  if (index >= 0) {
    list.splice(index, 1)
    return
...
<template>
  <div data-testid="settings-production-shell" class="h-[calc(100vh-57px)] overflow-hidden bg-[#f5f5f6] p-2 text-[#171717]">
    <template v-if="!formMode">
      <main data-testid="settings-production-card" class="relative mx-auto h-full w-full overflow-hidden rounded-lg border border-[#ececef] bg-white px-5 pb-5 pt-5 sm:px-8">
        ...
        <section class="mt-3 grid max-w-[684px] grid-cols-1 gap-3 sm:grid-cols-[240px_240px_60px_60px] sm:items-end sm:gap-x-6">
          ...
          <button type="button" class="h-8 rounded-lg border border-zinc-200 bg-white text-sm font-medium hover:bg-zinc-50" aria-label="清空智能体筛选" @click="clearFilters">清空</button>
          <button type="button" class="-ml-3 h-8 rounded-lg bg-[#171717] text-sm font-medium text-white hover:bg-black" aria-label="查询智能体" @click="applyFilters">查询</button>
        </section>
        ...
        <div class="mt-4 overflow-x-auto">
          <table class="w-full min-w-[820px] table-fixed border-collapse text-left">
            <thead class="bg-[#f7f7f8] text-sm font-medium text-zinc-500">
              <tr data-testid="settings-list-head" class="h-10">
            ...
            <tbody>
              <tr v-for="agent in filteredAgents" :key="agent.id" :data-testid="`settings-agent-row-${agent.id}`" class="h-14 border-b border-zinc-100 text-sm hover:bg-zinc-50/70">
                <td class="px-4">
                ...
                <td class="px-4">
                  <span class="inline-flex items-center gap-2 text-zinc-600">
                    <button
                      ...
                      :aria-label="`${agent.name}${agent.status === 'active' ? '已启用' : '已停用'}，点击切换`"
                      :data-testid="`settings-agent-switch-${agent.id}`"
                      class="relative h-5 w-9 shrink-0 rounded-full transition-colors"
                      :class="agent.status === 'active' ? 'bg-[#171717]' : 'bg-zinc-200'"
                      @click="toggleAgentStatus(agent)"
                    >
                      <i class="absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-all" :class="agent.status === 'active' ? 'left-[18px]' : 'left-0.5'" />
                    </button>
    ...
    <template v-else>
      <main data-testid="settings-production-card" class="relative mx-auto h-full w-full overflow-y-auto rounded-lg border border-[#ececef] bg-white px-5 pb-24 pt-4 sm:px-8">
            ...
            <section>
              ...
              <div class="mt-5 grid gap-5 sm:grid-cols-2">
                <fieldset class="min-w-0 border-0 p-0">
                  <legend class="text-sm font-medium">适用部门 <b class="text-red-500">*</b></legend>
                  <p class="mt-1 text-xs text-zinc-400">用于首页部门 Tab 筛选，不直接授予访问权限</p>
                  <div class="mt-2 flex flex-wrap gap-2">
                    <button v-for="department in departmentOptions" :key="department" type="button" class="rounded-full border px-3 py-1.5 text-xs" :class="draft.applicableDepartments.includes(department) ? 'border-blue-400 bg-blue-50 text-blue-600' : 'border-zinc-200 text-zinc-500'" @click="toggleArray(draft.applicableDepartments, department)">{{ department }}</button>
                  </div>
                </fieldset>
                <fieldset class="min-w-0 border-0 p-0">
                  <legend class="text-sm font-medium">接入系统标签</legend>
                  <p class="mt-1 text-xs text-zinc-400">首页卡片最多展示 3 个，每个标签不超过 6 个字符</p>
                  <div class="mt-2 flex flex-wrap gap-2">
                    <button v-for="system in systemOptions" :key="system" type="button" class="rounded-full border px-3 py-1.5 text-xs" :class="draft.connectedSystems.includes(system) ? 'border-blue-400 bg-blue-50 text-blue-600' : 'border-zinc-200 text-zinc-500'" @click="toggleLimited(draft.connectedSystems, system, 3)">{{ system }}</button>
                  </div>
                  <small class="mt-2 block text-zinc-400">已选择 {{ draft.connectedSystems.length }}/3，最多选择 3 个</small>
                </fieldset>
              </div>
