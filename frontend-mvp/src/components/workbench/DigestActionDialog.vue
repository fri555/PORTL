...
const todoPriority = ref<'high' | 'normal' | 'low'>('normal')
const todoTags = ref('')
const start = ref('')
const end = ref('')
...
function setDueDay(offset: number) {
  const value = new Date()
  value.setDate(value.getDate() + offset)
  value.setHours(18, 0, 0, 0)
  due.value = toLocalInput(value)
}
...
watch(() => [props.item.id, props.action.type], () => {
  ...
  todoTags.value = props.item.businessDomain || ''
...
const canSubmit = computed(() => {
  ...
  if (props.action.type === 'create_todo') return Boolean(title.value.trim() && executors.value.length && due.value)
      ...
      <div class="px-6 py-5">
        ...
        <template v-else-if="action.type === 'create_todo'">
          <div class="border-b border-[#edf0f4] pb-4">
            <label class="text-[12px] font-medium text-[#4d5560]">待办标题 <span class="text-red-500">*</span></label>
            <input v-model="title" data-testid="todo-title" class="mt-2 h-10 w-full border-0 border-b border-[#dfe4eb] px-0 text-[14px] font-medium text-[#252a31] outline-none placeholder:text-[#b0b5bd] focus:border-[#1769e0]" placeholder="写下你的待办事项" />
          </div>
          ...
          <div class="grid grid-cols-[22px_minmax(0,1fr)] gap-3 border-b border-[#edf0f4] py-4">
            <UsersRound class="mt-3 h-4 w-4 text-[#8b929c]" />
            <div class="space-y-3"><div><label class="text-[12px] font-medium text-[#4d5560]">执行人 <span class="text-red-500">*</span></label><DwsContactPicker v-model="executors" test-id="todo-executors" placeholder="从企业通讯录搜索执行人" class="mt-2" /></div><div><label class="text-[12px] font-medium text-[#4d5560]">参与人</label><DwsContactPicker v-model="participants" test-id="todo-participants" placeholder="添加协同参与人" class="mt-2" /></div></div>
          </div>
          <div class="grid grid-cols-[22px_minmax(0,1fr)] gap-3 border-b border-[#edf0f4] py-4">
            <CalendarClock class="mt-1 h-4 w-4 text-[#8b929c]" />
            <div>
              <label class="text-[12px] font-medium text-[#4d5560]">截止时间 <span class="text-red-500">*</span></label>
              <div class="mt-2 flex flex-wrap items-center gap-2">
                <button type="button" class="h-8 rounded-full border border-[#dfe4eb] px-3 text-[10px] font-medium text-[#59616c] hover:border-[#98b9eb] hover:text-[#1769e0]" @click="setDueDay(0)">今天 18:00</button>
                <button type="button" class="h-8 rounded-full border border-[#dfe4eb] px-3 text-[10px] font-medium text-[#59616c] hover:border-[#98b9eb] hover:text-[#1769e0]" @click="setDueDay(1)">明天 18:00</button>
                <input v-model="due" data-testid="todo-due" type="datetime-local" class="h-9 min-w-[210px] flex-1 rounded-xl border border-[#dfe4eb] px-3 text-[12px] text-[#3d444d] outline-none focus:border-[#78a8ef]" />
              </div>
            </div>
          </div>
          <div class="grid grid-cols-[22px_minmax(0,1fr)] gap-3 pt-4">
            <span class="mt-1 text-center text-[12px] text-[#8b929c]">◆</span>
            <div class="grid gap-3 sm:grid-cols-2">
              <div><label class="text-[12px] font-medium text-[#4d5560]">优先级 <span class="text-red-500">*</span></label><div data-testid="todo-priority" class="mt-2 grid grid-cols-3 gap-1 rounded-xl bg-[#f3f5f7] p-1"><button v-for="option in [{ value: 'high', label: '高' }, { value: 'normal', label: '普通' }, { value: 'low', label: '低' }]" :key="option.value" :data-testid="`todo-priority-${option.value}`" type="button" class="h-8 rounded-lg text-[10px] font-medium transition" :class="todoPriority === option.value ? 'bg-white text-[#1769e0] shadow-sm' : 'text-[#727a85] hover:text-[#3e4650]'" @click="todoPriority = option.value as 'high' | 'normal' | 'low'">{{ option.label }}</button></div></div>
              <div><label class="text-[12px] font-medium text-[#4d5560]">待办标签</label><input v-model="todoTags" data-testid="todo-tags" class="mt-2 h-10 w-full rounded-xl border border-[#dfe4eb] px-3 text-[12px] outline-none focus:border-[#78a8ef]" placeholder="多个标签用逗号分隔" /></div>
            </div>
          </div>
        </template>
