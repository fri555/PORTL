...
describe('DigestActionDialog', () => {
  ...
  it('shows the complete DingTalk todo fields', () => {
    ...
    expect(wrapper.find('[data-testid="todo-tags"]').exists()).toBe(true)
  })

  it('submits description, priority, participants, and tags with a todo', async () => {
    const wrapper = mountDialog({ type: 'create_todo', enabled: true })
    ...
    await wrapper.get('[data-testid="todo-tags"]').setValue('供应链, 履约')
