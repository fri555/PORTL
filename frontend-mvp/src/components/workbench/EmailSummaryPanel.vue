...
import { ArrowDown, ArrowUpRight, Inbox, Paperclip } from 'lucide-vue-next'
...
const filteredItems = computed(() => {
  ...
  return sourceItems.value.filter((item) => !keyword || [item.subject, item.senderName, item.senderEmail, item.summary, item.receivedAt, item.unread ? '未读' : '已读'].join(' ').toLowerCase().includes(keyword))
})
...
<template>
  <section data-testid="email-summary-panel" class="flex h-[316px] flex-col overflow-hidden rounded-[22px] border border-[#e7e9ee] bg-white shadow-[0_8px_30px_rgba(15,23,42,0.035)]">
    ...
    <div v-if="filteredItems.length" class="elegant-scrollbar min-h-0 flex-1 overflow-y-auto px-5" data-testid="email-summary-scroll-area">
      ...
      >
        ...
        <button type="button" class="min-w-0 rounded-xl text-left outline-none focus-visible:ring-2 focus-visible:ring-[#8fb5f3]" @click="openOriginal(item)">
          <div class="flex min-w-0 items-center gap-2">
            ...
            <span :data-testid="`email-read-state-${item.id}`" class="shrink-0 text-[9px] font-medium" :class="item.unread ? 'text-[#1769e0]' : 'text-[#a0a5ae]'">{{ item.unread ? '未读' : '已读' }}</span>
      ...
      <div v-if="hasMore" class="flex justify-center border-t border-[#eef0f3] py-2.5">
        <button
          data-testid="email-load-more"
          type="button"
          :disabled="loadingMore"
          class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-medium text-[#1769e0] transition hover:bg-[#f1f6ff] disabled:cursor-wait disabled:text-[#a0a6af]"
          @click="emit('loadMore')"
        >
          <ArrowDown class="h-3 w-3" :class="loadingMore ? 'animate-bounce' : ''" />
          {{ loadingMore ? '加载中…' : '加载更多邮件' }}
        </button>
      </div>
    </div>
