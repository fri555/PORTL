...
describe('useWorkbenchBoardState', () => {
  ...
  it('persists appended board data in the current browser session', () => {
    const state = useWorkbenchBoardState('live', 'email', vi.fn(), { items: [{ id: 'm1' }] })

    state.setData({ items: [{ id: 'm1' }, { id: 'm2' }] })

    expect(state.data.value.items).toEqual([{ id: 'm1' }, { id: 'm2' }])
    expect(JSON.parse(sessionStorage.getItem('workbench:live:email') ?? '{}').data.items).toEqual([{ id: 'm1' }, { id: 'm2' }])
  })
})
