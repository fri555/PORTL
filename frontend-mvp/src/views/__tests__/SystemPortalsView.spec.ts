...
describe('SystemPortalsView', () => {
  ...
  it('opens the focused AI todo generator and restores the manually entered draft', async () => {
    ...
    expect(closeConfirm.text()).toContain('确认关闭AI生成待办？')
    expect(closeConfirm.text()).toContain('输入文本已自动保存')
    expect(closeConfirm.text()).toContain('待办候选将不会保留')
  ...
  it('extracts multiple editable todo candidates and confirms only selected items', async () => {
    ...
    expect(dialog.get('[data-testid="toggle-all-meeting-todos"]').text()).toBe('清除')
    ...
    expect(dialog.get('[data-testid="ai-todo-dialog-body"]').classes()).toContain('h-[500px]')
