...
function mountWorkbench(role: UserRole = 'admin') {
  const pinia = createPinia()
  setActivePinia(pinia)
  useAppStore().login({ ...mockUser, role })
...
describe('SystemPortalsView', () => {
  ...
  it('switches one full-page module from a grouped side navigation', async () => {
    const wrapper = mountWorkbench()

    await wrapper.get('[data-testid="org-scope-open"]').trigger('click')

    expect(wrapper.get('[data-testid="workbench-nav-general"]').text()).toContain('通用功能')
    expect(wrapper.get('[data-testid="workbench-nav-management"]').text()).toContain('管理层')
    expect(wrapper.get('[data-testid="org-scope-sidebar"]').find('[data-testid="org-scope-node-management"]').exists()).toBe(true)
    expect(wrapper.get('[data-testid="workbench-module-system"]').attributes('style') ?? '').not.toContain('display: none')
    expect(wrapper.get('[data-testid="workbench-module-schedule"]').attributes('style')).toContain('display: none')

    await wrapper.get('[data-testid="workbench-nav-schedule"]').trigger('click')

    expect(wrapper.get('[data-testid="workbench-module-system"]').attributes('style')).toContain('display: none')
    expect(wrapper.get('[data-testid="workbench-module-schedule"]').attributes('style') ?? '').not.toContain('display: none')
  })

  it('configures schedule visibility through users, scopes, and the multi-source picker', async () => {
    const wrapper = mountWorkbench()

    await wrapper.get('[data-testid="workbench-nav-schedule"]').trigger('click')
    await wrapper.get('[data-testid="schedule-system-config"]').trigger('click')

    expect(wrapper.get('[data-testid="schedule-permission-dialog"]').text()).toContain('日程权限管理')
    expect(wrapper.get('[data-testid="schedule-permission-user-chen-le"]').findAll('button')).toHaveLength(1)
    ...
    expect(dialog.text()).toContain('按人选择')
    expect(dialog.text()).toContain('按群组选择')
    expect(dialog.text()).toContain('按架构选择')

    const linXiaCheckbox = dialog.findAll('label').find((label) => label.text().includes('林夏'))?.get('input[type="checkbox"]')
    expect(linXiaCheckbox).toBeTruthy()
    await linXiaCheckbox!.setValue(true)
  ...
  it('only exposes schedule permission configuration to administrators', async () => {
    const wrapper = mountWorkbench('user')
  ...
  it('isolates the DWS-backed 朝暮 identity from management mock data', async () => {
    ...
    expect(wrapper.find('[data-testid="schedule-load-summary"]').exists()).toBe(false)
  ...
  it('creates confirmed meeting todos in DingTalk only for a live personal workspace', async () => {
    ...
    await flushPromises()
    await dialog.get('[data-testid="toggle-all-meeting-todos"]').trigger('click')
    ...
    expect(JSON.parse(String(fetchMock.mock.calls[2]?.[1]?.body))).not.toHaveProperty('tags')
  ...
  it('renders the grouped full-page workbench without global search or create controls', () => {
    const wrapper = mountWorkbench()

    expect(wrapper.find('[data-testid="workbench-dashboard"]').exists()).toBe(true)
    expect(wrapper.get('[data-testid="workbench-nav-general"]').text()).toContain('系统入口')
    expect(wrapper.get('[data-testid="workbench-nav-general"]').text()).toContain('日程看板')
    expect(wrapper.get('[data-testid="workbench-nav-general"]').text()).toContain('待办中心')
    expect(wrapper.get('[data-testid="workbench-nav-management"]').text()).toContain('消息摘要')
    expect(wrapper.get('[data-testid="workbench-nav-management"]').text()).toContain('邮件摘要')
    expect(wrapper.get('[data-testid="workbench-nav-system"]').attributes('aria-current')).toBe('page')
    expect(wrapper.get('[data-testid="workbench-module-system"]').attributes('style') ?? '').not.toContain('display: none')
  ...
  it('renders Tencent Exmail as an independent one-email-per-row summary card', () => {
    ...
    expect(panel.findAll('[data-testid^="email-summary-item-"]')).toHaveLength(12)
  ...
  it('uses a collapsible organization sidebar that resizes the content area', async () => {
    ...
    expect(wrapper.get('[data-testid="org-scope-sidebar"]').classes()).toContain('md:w-16')
    expect(wrapper.get('[data-testid="org-scope-open"]').classes()).toContain('h-9')
    expect(wrapper.get('[data-testid="org-scope-open"]').classes()).toContain('w-9')
    ...
    await wrapper.get('[data-testid="org-scope-open"]').trigger('click')
    expect(wrapper.get('[data-testid="org-scope-sidebar"]').classes()).toContain('md:w-[260px]')
  ...
  it('starts with a wider calendar and keeps search in the right list toolbar', async () => {
    const wrapper = mountWorkbench()

    expect(wrapper.find('[data-testid="calendar-month-grid"]').exists()).toBe(true)
    expect(wrapper.get('[data-testid="schedule-layout"]').classes()).toContain('xl:grid-cols-[340px_minmax(0,1fr)]')
    expect(wrapper.get('[data-testid="schedule-list-pane"]').find('[aria-label="搜索日程看板"]').exists()).toBe(true)
  ...
  it('uses a shared time axis, hides load summary, and navigates adjacent days from the title', async () => {
    const wrapper = mountWorkbench()

    expect(wrapper.find('[data-testid="schedule-load-summary"]').exists()).toBe(false)
    const sidebar = wrapper.get('[data-testid="schedule-calendar-sidebar"]')
    const addPerson = sidebar.get('[data-testid="schedule-add-person"]')
    expect(addPerson.attributes('type')).toBe('button')
    expect(addPerson.element.parentElement?.className).toContain('border-t')
    expect(sidebar.text()).not.toContain('已显示的日历')
    expect(sidebar.text()).not.toContain('当前用户固定在首列')
    expect(wrapper.findAll('[data-testid="schedule-time-axis"]').length).toBe(1)
    expect(wrapper.findAll('[data-testid="schedule-time-block"]').length).toBeGreaterThan(0)
    expect(wrapper.find('[data-testid="schedule-now-line-mine"]').exists()).toBe(true)
    expect(wrapper.find('[data-testid="schedule-now-label"]').exists()).toBe(true)

    const selectedLabel = wrapper.get('[data-testid="schedule-selected-date"]').text()
    await wrapper.get('[aria-label="下一天"]').trigger('click')
    expect(wrapper.get('[data-testid="schedule-selected-date"]').text()).not.toBe(selectedLabel)
    expect(wrapper.find('[data-testid="schedule-now-line-mine"]').exists()).toBe(false)
    await wrapper.get('[aria-label="上一天"]').trigger('click')
    expect(wrapper.get('[data-testid="schedule-selected-date"]').text()).toBe(selectedLabel)
    expect(wrapper.find('[data-testid="schedule-now-line-mine"]').exists()).toBe(true)
  })

  it('positions schedules on fixed hourly intervals and spans long or overlapping meetings', () => {
    const wrapper = mountWorkbench()
    const axisLabels = wrapper.findAll('[data-testid="schedule-time-block"]').map((slot) => slot.attributes('data-time'))
    expect(axisLabels).toContain('09:00')
    expect(axisLabels).toContain('19:00')

    const longMeeting = wrapper.get('[data-testid="schedule-item-schedule-edge-long"]')
    expect(longMeeting.attributes('style')).toContain('height: 240px')
    expect(longMeeting.text()).toContain('12:00–15:00')

    const identicalA = wrapper.get('[data-testid="schedule-item-schedule-edge-same-range-1"]')
    const identicalB = wrapper.get('[data-testid="schedule-item-schedule-edge-same-range-2"]')
    expect(identicalA.attributes('style')).toContain('width: calc(25% - 8px)')
    expect(identicalB.attributes('style')).toContain('width: calc(25% - 8px)')
    expect(identicalA.attributes('data-conflict')).toBe('true')
    expect(identicalA.attributes('data-squeezed')).toBe('true')
    expect(identicalA.attributes('tabindex')).toBe('0')
    expect(identicalA.classes()).toContain('schedule-timeline-card')
    expect(wrapper.get('[data-testid="schedule-item-schedule-edge-different-end"]').text()).toContain('13:00–14:00')

    const shortMeeting = wrapper.get('[data-testid="schedule-item-schedule-edge-short"]')
    expect(shortMeeting.attributes('style')).toContain('height: 72px')
    expect(shortMeeting.text()).toContain('15:35–15:45')
    expect(shortMeeting.findAll('[data-testid="schedule-card-title-row"]')).toHaveLength(1)
    expect(shortMeeting.findAll('[data-testid="schedule-card-time-row"]')).toHaveLength(1)
    expect(shortMeeting.findAll('[data-testid="schedule-card-location-row"]')).toHaveLength(1)
    expect(shortMeeting.find('[data-testid="schedule-note-schedule-edge-short"]').exists()).toBe(true)

    const aiMeeting = wrapper.get('[data-testid="schedule-item-schedule-1"]')
    expect(aiMeeting.get('[data-testid="schedule-card-title-row"]').findAll('button')).toHaveLength(0)
    expect(aiMeeting.get('[data-testid="schedule-card-time-row"]').find('[data-testid="schedule-ai-detail-schedule-1"]').exists()).toBe(true)
    expect(aiMeeting.get('[data-testid="schedule-card-time-row"]').find('[data-testid="schedule-note-schedule-1"]').exists()).toBe(true)
    const dingtalkLink = aiMeeting.get('[data-testid="schedule-open-external-schedule-1"]')
    expect(dingtalkLink.attributes('target')).toBe('_blank')
    expect(dingtalkLink.attributes('aria-label')).toContain('在钉钉中查看')
    expect(wrapper.get('[data-testid="schedule-item-schedule-edge-contained"]').attributes('data-conflict')).toBe('true')
    expect(wrapper.get('[data-testid="schedule-item-schedule-edge-adjacent"]').attributes('data-conflict')).toBe('false')
  })

  it('reallocates an overlap cluster with a fixed 60/40 accordion and restores it safely', async () => {
    const wrapper = mountWorkbench()
    ...
    expect(sibling.attributes('style')).toContain('width: calc(13.3333% - 8px)')
    ...
    expect(expanded.attributes('style')).toContain('width: calc(13.3333% - 8px)')
  ...
  it('keeps wide calendar columns and scrolls horizontally beyond four people', async () => {
    const wrapper = mountWorkbench()

    for (const personId of ['liu-yang', 'chen-chen', 'zhao-liu']) {
      await wrapper.get('[data-testid="schedule-add-person"]').trigger('click')
      await wrapper.get(`[data-testid="schedule-person-option-${personId}"]`).trigger('click')
    }

    expect(wrapper.get('[data-testid="schedule-board-viewport"]').attributes('data-layout-mode')).toBe('adaptive')
    expect(wrapper.get('[data-testid="schedule-board-viewport"]').classes()).toContain('overflow-auto')
    expect(wrapper.get('[data-testid="schedule-board-track"]').attributes('style')).toContain('--board-count: 4')
    expect(wrapper.get('[data-testid="schedule-board-track"]').attributes('style')).toContain('--column-min-width: 300px')
    expect(wrapper.get('[data-testid="schedule-board-track"]').attributes('style')).toContain('--track-min-width: 1268px')
    expect(wrapper.findAll('.schedule-person-board')).toHaveLength(4)
    expect(wrapper.get('[data-testid="schedule-board-track"]').classes()).toContain('schedule-board-track--single-row')
    expect(wrapper.get('[data-testid="schedule-board-track"]').classes()).not.toContain('flex-wrap')
    expect(wrapper.get('[data-testid="schedule-board-mine"]').classes()).toContain('sticky')
    expect(wrapper.get('[data-testid="schedule-board-mine"]').find('[data-testid="schedule-person-header"]').exists()).toBe(true)
    expect(wrapper.get('[data-testid="schedule-item-schedule-1"]').classes()).toContain('bg-[#fff0ed]')
    expect(wrapper.get('[data-testid="schedule-item-schedule-1"]').attributes('data-conflict')).toBe('true')
    expect(wrapper.get('[data-testid="schedule-item-schedule-2"]').classes()).toContain('bg-[#fff8df]')
    expect(wrapper.get('[data-testid="schedule-item-schedule-completed-review"]').classes()).toContain('bg-[#f3f4f6]')
    expect(wrapper.get('[data-testid="schedule-add-person"]').attributes('disabled')).toBeUndefined()

    await wrapper.get('[data-testid="schedule-add-person"]').trigger('click')
    await wrapper.get('[data-testid="schedule-person-option-sun-qi"]').trigger('click')

    expect(wrapper.get('[data-testid="schedule-board-viewport"]').attributes('data-layout-mode')).toBe('overflow')
    expect(wrapper.get('[data-testid="schedule-board-viewport"]').classes()).toContain('overflow-auto')
    expect(wrapper.get('[data-testid="schedule-board-track"]').attributes('style')).toContain('--track-min-width: 1568px')
    expect(wrapper.get('[data-testid="schedule-board-mine"]').classes()).toContain('sticky')
    expect(wrapper.findAll('.schedule-person-board')).toHaveLength(5)
  })

  it('reorders peer calendars with drag and drop', async () => {
    const wrapper = mountWorkbench()
    ...
    expect(wrapper.findAll('[data-testid^="schedule-peer-board-"]').map((item) => item.attributes('data-testid'))).toEqual([
      'schedule-peer-board-liu-yang',
      'schedule-peer-board-chen-chen',
      'schedule-peer-board-zhao-liu',
    ])

    await wrapper.get('[data-testid="schedule-sidebar-person-liu-yang"]').trigger('dragstart')
    await wrapper.get('[data-testid="schedule-sidebar-person-zhao-liu"]').trigger('drop')
    expect(wrapper.findAll('[data-testid^="schedule-peer-board-"]').map((item) => item.attributes('data-testid'))).toEqual([
      'schedule-peer-board-chen-chen',
      'schedule-peer-board-zhao-liu',
      'schedule-peer-board-liu-yang',
    ])
    expect(wrapper.findAll('[data-testid^="schedule-sidebar-person-"]').map((item) => item.attributes('data-testid'))).toEqual([
      'schedule-sidebar-person-chen-chen',
      'schedule-sidebar-person-zhao-liu',
      'schedule-sidebar-person-liu-yang',
    ])
  })
  ...
  it('opens a full note editor, discards unsaved drafts, and restores only saved versions', async () => {
    const wrapper = mountWorkbench()
    ...
    const dialog = wrapper.get('[data-testid="meeting-notes-dialog"]')
    expect(dialog.text()).toContain('日程笔记')
    expect(dialog.find('[data-testid="meeting-note-title"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="meeting-note-toolbar"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="todo-candidate-panel"]').exists()).toBe(false)
    expect(dialog.get('[data-testid="meeting-note-editor"]').attributes('placeholder')).toBe('记录会议内容、结论和后续事项…')
    ...
    expect(dialog.get('[data-testid="meeting-note-save-state"]').text()).toContain('未保存')
    expect(document.body.style.overflow).toBe('hidden')

    await dialog.get('[aria-label="关闭日程笔记"]').trigger('click')
    const closeConfirm = dialog.get('[data-testid="ai-todo-close-confirm"]')
    expect(closeConfirm.text()).toContain('确认关闭日程笔记？')
    expect(closeConfirm.text()).toContain('未保存的标题和正文将丢失')
    await closeConfirm.get('[data-testid="cancel-ai-todo-close"]').trigger('click')
    expect(wrapper.find('[data-testid="meeting-notes-dialog"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="ai-todo-close-confirm"]').exists()).toBe(false)
    ...
    await wrapper.get('[data-testid="schedule-note-schedule-1"]').trigger('click')
    expect(wrapper.get('[data-testid="meeting-note-editor"]').element).toHaveProperty('value', '')
    await wrapper.get('[data-testid="meeting-note-title"]').setValue('需求评审纪要')
    await wrapper.get('[data-testid="meeting-note-editor"]').setValue('保存后的会议结论')
    await wrapper.get('[data-testid="save-meeting-note"]').trigger('click')
    expect(wrapper.get('[data-testid="meeting-note-save-state"]').text()).toContain('笔记已保存')
    await wrapper.get('[aria-label="关闭日程笔记"]').trigger('click')
    expect(wrapper.find('[data-testid="meeting-notes-dialog"]').exists()).toBe(false)
    await wrapper.get('[data-testid="schedule-note-schedule-1"]').trigger('click')
    expect(wrapper.get('[data-testid="meeting-note-title"]').element).toHaveProperty('value', '需求评审纪要')
    expect(wrapper.get('[data-testid="meeting-note-editor"]').element).toHaveProperty('value', '保存后的会议结论')
  })

  it('keeps the note action while expressing status through card color', () => {
    const wrapper = mountWorkbench()
    const schedule = wrapper.get('[data-testid="schedule-item-schedule-completed-review"]')

    expect(schedule.find('[data-testid="schedule-note-schedule-completed-review"]').exists()).toBe(true)
    expect(schedule.get('[data-testid="schedule-note-schedule-completed-review"]').text()).toBe('笔记')
    expect(schedule.text()).not.toContain('已结束')
    expect(schedule.classes()).toContain('bg-[#f3f4f6]')
  })
  ...
  it('extracts multiple editable todo candidates and confirms only selected items', async () => {
    ...
    expect(dialog.findAll('[data-testid^="meeting-todo-selected-"]').every((item) => (item.element as HTMLInputElement).checked === false)).toBe(true)
    expect(dialog.text()).not.toContain('已选')
    expect(dialog.get('[data-testid="toggle-all-meeting-todos"]').text()).toBe('全选')
    expect(dialog.get('[data-testid="confirm-meeting-todos"]').attributes('disabled')).toBeDefined()
    ...
    expect(dialog.get('[data-testid="toggle-all-meeting-todos"]').text()).toBe('清空')
    ...
    expect(dialog.get('[data-testid="confirm-meeting-todos"]').attributes('disabled')).toBeDefined()
    expect(dialog.get('[data-testid="toggle-all-meeting-todos"]').text()).toBe('全选')
    await dialog.get('[data-testid="meeting-todo-selected-1"]').setValue(true)
    await dialog.get('[data-testid="meeting-todo-selected-2"]').setValue(true)

    expect(dialog.get('[data-testid="ai-todo-dialog-body"]').classes()).toContain('h-[560px]')
    expect(dialog.get('[data-testid="todo-candidate-panel"]').classes()).toContain('min-h-0')
    expect(dialog.get('[data-testid="todo-candidate-scroll"]').classes()).toContain('overflow-y-auto')

    await dialog.get('[data-testid="edit-meeting-todo-1"]').trigger('click')
    expect(dialog.get('[data-testid="meeting-todo-candidate-1"]').text()).toContain('提醒时间')
    expect(dialog.get('[data-testid="meeting-todo-candidate-1"]').findAll('span').filter((item) => item.text() === '*')).toHaveLength(1)
    ...
    expect(dialog.find('[data-testid="meeting-todo-due-1"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="meeting-todo-reminder-1"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="meeting-todo-priority-1"]').exists()).toBe(true)
    expect(dialog.get('[data-testid="meeting-todo-priority-1"]').findAll('option').map((option) => option.text())).toEqual(['较低', '普通', '较高', '紧急'])
    expect(dialog.find('[data-testid="meeting-todo-tags-1"]').exists()).toBe(false)
    expect(dialog.get('[data-testid="meeting-todo-title-priority-row-1"]').classes()).toContain('grid-cols-[minmax(0,1fr)_112px]')
    expect(dialog.get('[data-testid="meeting-todo-people-row-1"]').classes()).toContain('grid-cols-2')
    expect(dialog.get('[data-testid="meeting-todo-participants-1"]').text()).toContain('当前用户')
    expect(dialog.get('[data-testid="meeting-todo-time-row-1"]').classes()).toContain('grid-cols-2')
    expect(dialog.find('[data-testid="meeting-todo-due-1-date"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="meeting-todo-due-1-time"]').exists()).toBe(true)
  ...
  it('keeps a focused note toolbar and explicit save control without legacy OneNote actions', async () => {
    const wrapper = mountWorkbench()
    ...
    expect(dialog.find('[data-testid="save-meeting-note"]').exists()).toBe(true)
    expect(dialog.find('[data-testid="meeting-note-toolbar"]').exists()).toBe(true)
  ...
  it('hides schedule load and visible action labels while retaining pre-meeting context', async () => {
    const wrapper = mountWorkbench()

    expect(wrapper.find('[data-testid="schedule-load-summary"]').exists()).toBe(false)
    expect(wrapper.find('[data-testid^="schedule-action-"]').exists()).toBe(false)
