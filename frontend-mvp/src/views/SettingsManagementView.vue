...
> = {
  quota: {
    label: '额度管理',
    subtitle: '按部门、角色和场景分配人民币与 Token 预算',
...
const records = reactive<Record<SettingDomain, SettingRecord[]>>({
  ...
  tool: [
    ...
    },
    {
      ...
      secondary: 'API Key',
      limit: '外网审批',
      updatedAt: '07-16 19:30',
      tags: ['Tavily', 'Firecrawl'],
    },
  ],
  agent: [
    {
    ...
    },
    {
      ...
      status: 'active',
      primary: '单专家',
      secondary: 'Qwen3 企业主模型',
      limit: 'Excel/Pandas/RAG',
      updatedAt: '昨天 13:44',
      tags: ['报表', '归因'],
    },
    {
...
const pageTitle = computed(() =>
  formMode.value
    ? `${formMode.value === 'create' ? '新建' : '编辑'}${currentMeta.value.label.replace('管理', '')}`
    : currentMeta.value.label,
)
const agentPublishSummary = computed(() => {
  ...
  return [
    {
      label: '已发布',
      value: `${activeAgents.length}/${agentRecords.length}`,
      detail: '仅启用中的智能体对员工可见',
    },
    {
      label: '待审核',
      value: String(agentRecords.filter((item) => item.status === 'draft').length),
      detail: '草稿需补齐 Prompt、工具和知识引用',
    },
...
<template>
  ...
  <div v-else class="min-h-[calc(100vh-3.5rem)] bg-[#f6f6f7] text-[#111]">
    <div class="flex min-h-[calc(100vh-3.5rem)] w-full">
      <aside
        class="hidden w-[256px] shrink-0 border-r border-[#e5e5e7] bg-white px-3 py-5 lg:block"
      >
        <div class="px-2">
        ...
        <nav class="mt-5 space-y-1">
          <button
            ...
            :class="
              activeDomain === key
                ? 'bg-[#111] text-white'
                : 'text-zinc-600 hover:bg-[#f4f4f5] hover:text-zinc-950'
            "
      ...
      <main class="min-w-0 flex-1">
        <template v-if="!formMode">
          <section class="w-full border-b border-[#e5e5e7] bg-white px-5 py-5 sm:px-8">
            <div class="flex flex-wrap items-start gap-4">
              ...
              <button
                type="button"
                class="inline-flex h-9 items-center gap-2 rounded-lg bg-[#111] px-4 text-sm font-medium text-white hover:bg-[#333]"
                @click="openCreate"
              >
                <Plus class="h-4 w-4" />
          ...
          <section class="w-full px-5 py-5 sm:px-8">
            ...
            <div
              class="mt-4 flex flex-col gap-3 rounded-lg border border-[#e5e5e7] bg-white p-3 sm:flex-row sm:items-center"
            >
              <div
                class="flex h-9 min-w-0 flex-1 items-center gap-2 rounded-lg border border-[#e5e5e7] bg-[#f8f8f9] px-3"
              >
                <Search class="h-4 w-4 text-zinc-400" />
                <input
                  v-model="query"
                  class="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-zinc-400"
                  placeholder="搜索名称、负责人、范围或标签"
                />
              </div>
              ...
              <div
                class="inline-flex h-9 rounded-lg border border-[#e5e5e7] bg-white p-1"
                aria-label="展示方式"
              >
                <button
                  type="button"
            ...
            <div v-if="viewMode === 'card'" class="mt-4 grid gap-3 xl:grid-cols-2 2xl:grid-cols-3">
              ...
              >
                <div class="flex items-start gap-3">
                  <div
                    class="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[#f4f4f5] text-zinc-700"
                  >
                    <component :is="currentMeta.icon" class="h-5 w-5" />
                  </div>
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-2">
                      ...
                      <span
                        class="shrink-0 rounded-full px-2 py-0.5 text-[11px] font-medium ring-1"
                        :class="statusClass(item.status)"
                        >{{ statusText(item.status) }}</span
                      >
                    </div>
                    <p class="mt-1 truncate text-xs text-zinc-400">
                      {{ item.owner }} · {{ item.scope }} · {{ item.updatedAt }}
                    </p>
                  </div>
                  <div class="relative shrink-0">
                    <button
                      type="button"
                      class="grid h-8 w-8 place-items-center rounded-lg text-zinc-400 hover:bg-zinc-100 hover:text-zinc-800"
                    ...
                    <div
                      v-if="menuId === item.id"
                      class="absolute right-0 top-9 z-40 w-36 overflow-hidden rounded-lg border border-zinc-200 bg-white p-1 shadow-xl"
                    >
                      <button
                        type="button"
                        class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-zinc-700 hover:bg-zinc-50"
                        @click="openEdit(item)"
                      >
                      ...
                      <button
                        type="button"
                        class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs text-red-600 hover:bg-red-50"
                ...
                <div class="mt-5 grid gap-3 sm:grid-cols-3">
                  <div class="rounded-lg bg-[#f8f8f9] px-3 py-2">
                    ...
                    <div class="mt-1 truncate text-sm font-medium text-zinc-800">
                      {{ item.primary }}
                    </div>
                  </div>
                  <div class="rounded-lg bg-[#f8f8f9] px-3 py-2">
                ...
                <div class="mt-4 flex flex-wrap gap-1.5">
                  <span
                    ...
                    :key="tag"
                    class="rounded-md bg-zinc-100 px-2 py-1 text-[11px] text-zinc-500"
                    >{{ tag }}</span
                  >
                </div>
              </article>
            ...
            <div v-else class="mt-4 overflow-hidden rounded-lg border border-[#e5e5e7] bg-white">
              <div
                class="grid min-h-11 grid-cols-[minmax(260px,1.5fr)_120px_minmax(150px,1fr)_minmax(160px,1fr)_120px_72px] items-center border-b border-[#ededee] bg-[#fbfbfc] px-4 text-xs font-medium text-zinc-500"
              >
                <span>名称</span>
                <span>状态</span>
              ...
              >
                <div class="min-w-0">
                  ...
                  <div class="mt-1 flex flex-wrap items-center gap-1.5 text-xs text-zinc-400">
                    ...
                    <span
                      v-for="tag in item.tags"
                      :key="tag"
                      class="rounded-md bg-zinc-100 px-1.5 py-0.5 text-[10px] text-zinc-500"
                      >{{ tag }}</span
                    >
                  </div>
                </div>
                <div>
                  <span
                    class="inline-flex rounded-full px-2 py-1 text-xs font-medium ring-1"
                    :class="statusClass(item.status)"
                    >{{ statusText(item.status) }}</span
                  >
                </div>
        ...
        <template v-else>
          <section
            class="grid min-h-[calc(100vh-3.5rem)] grid-cols-1 bg-white lg:grid-cols-[240px_minmax(520px,760px)_1fr]"
          >
            <aside
              class="border-b border-[#e5e5e7] bg-[#fbfbfc] px-5 py-5 lg:border-b-0 lg:border-r"
            >
              <button
                type="button"
                class="inline-flex h-8 items-center gap-1.5 rounded-lg px-2 text-sm text-zinc-600 hover:bg-zinc-100"
                @click="closeForm"
              >
              ...
              <div class="mt-6 space-y-3">
                <div
                  v-for="(step, index) in ['基础信息', '能力配置', '权限与发布']"
                  :key="step"
                  class="flex gap-3"
                >
                  <div
            ...
            <section class="border-r border-[#e5e5e7] px-6 py-6">
              ...
              <div class="mt-6 space-y-5">
                <label class="block">
                  ...
                  <input
                    v-model="draft.name"
                    class="mt-2 h-10 w-full rounded-lg border border-zinc-200 px-3 text-sm outline-none focus:border-zinc-400"
                    placeholder="输入名称"
                  />
                </label>
                <div class="grid gap-4 sm:grid-cols-2">
                  ...
                  <label class="block">
                    ...
                    <input
                      v-model="draft.scope"
                      class="mt-2 h-10 w-full rounded-lg border border-zinc-200 px-3 text-sm outline-none focus:border-zinc-400"
                      placeholder="部门、角色或项目"
                    />
                  </label>
                </div>
                <label class="block">
                  <span class="text-xs font-medium text-zinc-500">{{
                    currentMeta.fields.primary
                  }}</span>
                  <input
                ...
                <label class="block">
                  <span class="text-xs font-medium text-zinc-500">{{
                    currentMeta.fields.limit
                  }}</span>
                  <input
                    v-model="draft.limit"
                    class="mt-2 h-10 w-full rounded-lg border border-zinc-200 px-3 text-sm outline-none focus:border-zinc-400"
                    placeholder="额度、窗口或权限范围"
                  />
                </label>
                <label class="block">
              ...
              <div class="mt-7 flex justify-end gap-3 border-t border-zinc-100 pt-5">
                <button
                  type="button"
                  class="h-9 rounded-lg border border-zinc-200 px-4 text-sm text-zinc-600 hover:bg-zinc-50"
                  @click="closeForm"
                >
                  取消
                </button>
                <button
                  type="button"
                  class="h-9 rounded-lg bg-[#111] px-4 text-sm font-medium text-white hover:bg-[#333]"
                  @click="saveRecord"
                >
                  保存
                </button>
            ...
            <aside class="hidden bg-[#fbfbfc] px-6 py-6 lg:block">
              <div
                class="rounded-lg border border-dashed border-zinc-300 bg-white/60 p-6 text-center"
              >
                <div
                  class="mx-auto grid h-11 w-11 place-items-center rounded-lg bg-zinc-100 text-zinc-400"
                >
                  <ShieldCheck class="h-5 w-5" />
                </div>
                <p class="mt-3 text-sm font-medium text-zinc-700">右侧扩展区</p>
                <p class="mt-1 text-xs leading-5 text-zinc-400">
                  后续可放预览、审批流、调用日志、版本差异或智能评估结果。
                </p>
              </div>
            </aside>
          </section>
    ...
    >
      <div
        v-if="toast"
        class="fixed left-1/2 top-20 z-50 flex -translate-x-1/2 items-center gap-2 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-800 shadow-xl"
      >
        <KeyRound class="h-4 w-4 text-zinc-400" />
        <span>{{ toast }}</span>
        <button
          type="button"
          class="rounded-md p-1 text-zinc-400 hover:bg-zinc-100"
          @click="toast = ''"
        >
          <X class="h-3.5 w-3.5" />
        </button>
