...
<template>
  <div class="container py-6">
    <div class="mx-auto max-w-3xl">
      ...
      <article v-else>
        <div class="overflow-hidden rounded-xl bg-zinc-900">
          <img :src="article.coverImage" :alt="article.title" class="aspect-[21/9] w-full object-cover" />
        </div>
        ...
        <h1 class="mt-3 text-balance text-2xl font-bold leading-tight md:text-3xl">{{ article.title }}</h1>
        <p class="mt-3 text-[15px] leading-relaxed text-muted-foreground">{{ article.summary }}</p>

        <div class="mt-5 flex flex-wrap items-center gap-4 border-y py-3 text-sm">
        ...
        <!-- action bar -->
        <div class="mt-6 flex items-center gap-3 rounded-xl border bg-white p-3 shadow-sm">
          <Button
    ...
    >
      <div
        v-if="copied"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 rounded-lg bg-zinc-900 px-4 py-2 text-sm text-white shadow-lg"
      >
        链接已复制到剪贴板
      </div>
    </Transition>
