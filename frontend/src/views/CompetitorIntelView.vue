...
<template>
  <PageContainer title="竞对情报" :breadcrumb="['首页', '竞对情报']">
    ...
    <div v-if="threats.length && (sub === 'all' || sub === '威胁预警')" class="mb-6 space-y-3">
      <h3 class="flex items-center gap-2 text-sm font-semibold text-red-600">
        <ShieldAlert class="h-4 w-4" /> 威胁预警
      </h3>
    ...
    <div class="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center">
      <div class="no-scrollbar flex items-center gap-1 overflow-x-auto rounded-lg border bg-white p-1">
        <button
          v-for="t in subTabs"
          :key="t.key"
          :class="
      ...
      <div class="relative lg:ml-auto lg:w-64">
        ...
        <Input v-model="search" placeholder="搜索情报…" class="h-10 pl-9" />
      </div>
    </div>
